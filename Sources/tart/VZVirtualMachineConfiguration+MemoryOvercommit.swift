...
extension VZVirtualMachineConfiguration {
  ...
  func allowMemoryOvercommitment() throws {
    guard Self.supportsMemoryOvercommitment,
          let setterImplementation = method(for: Self.memoryOvercommitmentSetter) else {
      throw MemoryOvercommitError.unavailable
    }

    typealias Setter = @convention(c) (AnyObject, Selector, Bool) -> Void
    unsafeBitCast(setterImplementation, to: Setter.self)(self, Self.memoryOvercommitmentSetter, true)

    guard isMemoryOvercommitmentAllowed == true else {
